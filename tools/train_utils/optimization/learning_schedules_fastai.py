...
class RampUpWarmupLR(lr_sched._LRScheduler):
    # only for DVP
    def __init__(self, optimizer, T_max, eta_min=0, last_epoch=-1):
        self.T_max = T_max
        self.eta_min = 0.001333333
        super(RampUpWarmupLR, self).__init__(optimizer, last_epoch)

    def get_lr(self):
        return [self.eta_min + (0.0015 - self.eta_min) * 
